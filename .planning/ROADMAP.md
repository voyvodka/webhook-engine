...
- [x] **Phase 1: Safety Foundations** - Correctness defects fixed: circuit breaker race condition eliminated, message status transitions guarded, stale lock recovery hardened, signing secret enforced at the data layer (completed 2026-03-30)
- [x] **Phase 2: Security Hardening** - Public endpoints rate-limited, replay scope threat model documented, idempotency window made configurable (completed 2026-03-30)
...
Plans:
- [x] 01-01-PLAN.md — Circuit breaker race condition fix with advisory locks + CORR-04 stale lock verification
- [x] 01-02-PLAN.md — Message status state machine to prevent Delivered-to-Pending regression
...
Plans:
- [x] 02-01-PLAN.md — Per-AppId token bucket rate limiting on Send and BatchSend endpoints
- [x] 02-02-PLAN.md — Replay scope threat model ADR
- [x] 02-03-PLAN.md — Configurable idempotency window per application
...
Plans:
- [x] 03-01-PLAN.md — ADR-002 CQRS scaffold removal + Application layer cleanup + RateLimitResolver extraction
- [x] 03-02-PLAN.md — DashboardController split into 4 single-responsibility controllers
- [x] 03-03-PLAN.md — Post-release triage flow + v0.1.1 stabilization backlog
- [ ] 03-04-PLAN.md — DevTrafficGenerator decomposition into orchestrator + TrafficScheduler + EndpointTrafficProfiler
...
|-------|----------------|--------|-----------|
| 1. Safety Foundations | 3/3 | Complete   | 2026-03-30 |
| 2. Security Hardening | 3/3 | Complete   | 2026-03-30 |
| 3. Structural Cleanup | 2/4 | In Progress|  |
